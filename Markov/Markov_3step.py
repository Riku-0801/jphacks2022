...
def create_words_cnt_dic(datas):
    ...
    words = []
    for data in datas:
        words.extend(list(ngrams(data, 4)))
...
def create_m_dic(words_cnt_dic):
    ...
    for k, v in words_cnt_dic.items():
        # 先頭２単語、その次の単語
        two_words, next_word = k[:3], k[3]
        f_two_words, f_next_word = k[:2],k[2]

        if f_two_words not in f_m_dic:
            f_m_dic[f_two_words] = {'words': [], 'weights': []}

        # 先頭２単語に対し、次に来る単語とその重み
        f_m_dic[f_two_words]['words'].append(f_next_word)
        f_m_dic[f_two_words]['weights'].append(v)
...
def create_sentences(f_m_dic, m_dic, begin_words, begin_weights):
    ...
    words, weights = f_m_dic[back_words]['words'], f_m_dic[back_words]['weights']
    next_word = random.choices(words, weights=weights, k=1)[0]
    sentences.append(next_word)
    # 作成文章の後方２単語をもとに、次の単語を抽選する
    while True:
        # 後方２単語
        back_words = tuple(sentences[-3:])
        # マルコフ用辞書から抽選
...
# マルコフ用辞書作成
m_dic,f_m_dic = create_m_dic(words_cnt_dic)
...
# 20回 文章生成
for i in range(20):
    text = create_sentences(f_m_dic,m_dic, begin_words, begin_weights)
